...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    array = np.linspace(low_heuristic_weight,high_heuristic_weight, n)
    ...
    for w in array:
        ...
        if res.is_solution_found:
            costList.append(res.solution_g_cost)
            expandedList.append(res.nr_expanded_states)
            weightList.append(w)
    plot_distance_and_expanded_wrt_weight_figure("GRAPH",weightList,costList,expandedList)
...
def mda_problem_with_astar_experiments():
    ...
    as1 = AStar(MDAMaxAirDistHeuristic)
    res = as1.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
    ...
    moderate_mda_problem_with_distance_cost = get_mda_problem('moderate', MDAOptimizationObjective.Distance)
    as2 = AStar(MDASumAirDistHeuristic)
    res2 = as2.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res2)
    ...
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    as1 = AStar(MDAMSTAirDistHeuristic)
    res = as1.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
...
def multiple_objectives_mda_problem_experiments():
    ...
    as1 = AStar(MDATestsTravelDistToNearestLabHeuristic)
    res = as1.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
...
def mda_problem_with_astar_epsilon_experiments():
    ...
    astareps = AStarEpsilon(MDAMSTAirDistHeuristic,within_focal_priority_function=within_focal_h_sum_priority_function,focal_epsilon=0.23,max_focal_size=40)
    res = astareps.solve_problem(small_mda_problem_with_distance_cost)
    print(res)
